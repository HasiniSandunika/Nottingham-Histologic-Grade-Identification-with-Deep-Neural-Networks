...
print("Four Predict Model Implementation")
...
def create_numpy(read_path, save_path, set_type, value):
  ...
  for i in images:
    ...
    if int(i[value:value + 1]) == 0:
      r = 0
    elif int(i[value:value + 1]) == 1:
      r = 1
    elif int(i[value:value + 1]) == 2:
      r = 2
    else:
      r = 3   
    y.append(r)
...
create_numpy('/four_predict_model/dataset/train_data', '/four_predict_model/numpy', 'train', 72) 

## create numpy arrays for testing dataset from processed images
create_numpy('/four_predict_model/dataset/test_data', '/four_predict_model/numpy', 'test', 71) 

## split training dataset for training and validation
x_train_rgb = np.load('/four_predict_model/numpy/x_train.npy')
y_train_rgb = np.load('/four_predict_model/numpy/y_train.npy')
...
filepath='/four_predict_model/models/weights25.h5'
...
model.save('/four_predict_model/models/model.h5')
...
model = load_model('/four_predict_model/models/weights25.h5')
model.summary()
